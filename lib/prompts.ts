...
# ステップ1: 検索（最大2回まで。スキップ禁止・速度優先のためWebFetchは使わない）
以下を検索し、最新話の話数とあらすじを確認してください:
- 「週刊少年ジャンプ ワンピース 最新話 ${today.slice(0, 7)}」
- 「ワンピース 最新話 あらすじ」
- 「ネタバレ確定速報」のような煽り気味のタイトルは、まだ発売・掲載されていない次号の内容を先読みして書いている場合がある。話数は複数の検索結果で一致した数字を採用し、1つの記事にしか出てこず「確定速報」「予想」等と書かれているだけの話数は避ける（迷ったら1つ低い方の話数を採用する）。
- 学習データの記憶だけで話数を決めず、必ず検索結果を根拠にすること。
...
# ステップ2: クイズ作成
ステップ1で確認した最新情報を元に、4択クイズを${count}問作成してください。
