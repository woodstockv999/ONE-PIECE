...
# ステップ1: 最新話数の特定（検索必須・スキップ禁止）
まず「現時点で週刊少年ジャンプに掲載されている最新話が何話か」を検索で特定してください。
- 検索クエリ例:「ワンピース 最新話 何話 ${today.slice(0, 7)}」「週刊少年ジャンプ ワンピース 最新話 話数」
- 学習データの記憶だけで話数を決めない。必ず検索結果に明記された話数の数字を採用すること。
- 複数の検索結果を確認し、話数の数字が一致することを確認してから確定する。結果によって数字が食い違う場合は、最も日付が新しく、かつ複数の情報源で一致する数字を採用する。単一の情報源にしか出てこない数字だけを根拠にしない。
- 確定した話数を基準に、それ以降（未掲載）の内容は絶対に出題しない。
