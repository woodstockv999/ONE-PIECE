...
/**
 * 最新話モード（Web検索あり）のクイズ生成プロンプト。
 * 知識カットオフ以降の最新の展開からも出題する。
 */
export function buildLatestQuizPrompt(
  ...
  return `あなたは漫画「ONE PIECE」（尾田栄一郎・集英社）に精通したクイズ作成者です。
web_search ツールを使い、ONE PIECE の「最新話・直近の展開」に関する最新情報を調べたうえで、
以下の条件で4択クイズを${count}問作成してください。

# 条件
- 難易度: ${difficulty}（${DIFFICULTY_GUIDE[difficulty]}）
- カテゴリ: ${category}
- 直近の連載（最新話・最近のアーク）に関する内容を中心に出題する。
...
# 出力形式（厳守）
- 最後に JSON のみを出力する。前置き・説明・Markdownのコードフェンス（\`\`\`）を付けない。
- スキーマ:
{
  "questions": [
    {
