...
APP_DIR="$HOME/apps/one-piece"
BRANCH="main"
LOCK_FILE="/tmp/onepiece-update.lock"
LOG_FILE="$HOME/logs/onepiece-update.log"
...
if bash "$APP_DIR/deploy/deploy.sh" >> "$LOG_FILE" 2>&1; then
  echo "[$(date '+%F %T')] デプロイ完了" >> "$LOG_FILE"
  send_mail "[ONE PIECE] デプロイ完了 ✅" "$(printf '%s -> %s がデプロイされました。\n\n時刻: %s\nURL: http://210.131.212.62/onepiece' "${LOCAL:0:7}" "${REMOTE:0:7}" "$(date '+%F %T')")"
else
  echo "[$(date '+%F %T')] ERROR: デプロイ失敗（ログを確認してください）" >> "$LOG_FILE"
  send_mail "[ONE PIECE] デプロイ失敗 ❌" "$(printf '%s -> %s のデプロイに失敗しました。\n\n時刻: %s\nログ: %s' "${LOCAL:0:7}" "${REMOTE:0:7}" "$(date '+%F %T')" "$LOG_FILE")"
  exit 1
fi
